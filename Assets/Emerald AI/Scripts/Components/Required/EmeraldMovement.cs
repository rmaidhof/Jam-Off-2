    ...
    public class EmeraldMovement : MonoBehaviour
    {
        ...
        void MoveAINavMesh()
        {
            ...
            if (AIAgentActive && m_NavMeshAgent.isOnNavMesh && MovementInitialized)
            {
                ...
                if (AIAnimator.GetBool("Hit") || m_NavMeshAgent.remainingDistance <= m_NavMeshAgent.stoppingDistance || CanIdle())
                {
                    AIAnimator.SetFloat("Speed", 0, 0.3f, Time.deltaTime);
                    m_NavMeshAgent.speed = 0;
                }
                else if (!AIAnimator.GetBool("Hit") && m_NavMeshAgent.remainingDistance > m_NavMeshAgent.stoppingDistance && CanMove())
                {
        ...
        void MoveAIRootMotion()
        {
            ...
            if (AIAgentActive && m_NavMeshAgent.isOnNavMesh && MovementInitialized)
            {
                ...
                if (RotateTowardsTarget || AIAnimator.GetBool("Hit") || DestinationAdjustedAngle > MovingTurnLimit || m_NavMeshAgent.remainingDistance + 0.2f <= m_NavMeshAgent.stoppingDistance || CanIdle())
                {
        ...
        void NextWaypoint()
        {
            ...
            if (WaypointType != WaypointTypes.Random && WaypointsList.Count > 1 && !WaypointReverseActive)
            {
