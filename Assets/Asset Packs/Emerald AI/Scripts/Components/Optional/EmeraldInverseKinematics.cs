...
namespace EmeraldAI
{
    ...
    public class EmeraldInverseKinematics : MonoBehaviour
    {
        ...
        void UpdateIK ()
        {
            ...
            if (EmeraldComponent.CurrentTargetInfo.TargetSource != null)
            {
                ...
                if (CurrentTargetAngle <= LookAtLimit && Distance < LookDistance && !EmeraldComponent.AnimationComponent.IsStunned && !EmeraldComponent.DetectionComponent.TargetObstructed && !EmeraldComponent.AnimationComponent.IsTurning && !EmeraldComponent.AnimationComponent.IsDodging)
                {
                    if (Distance > 0.75f)
                    {
                        float CurrentLookDistance = Vector3.Distance(new Vector3(m_AimSource.position.x, m_AimSource.position.y, CurrentTargetPosition.z), new Vector3(CurrentTargetPosition.x, CurrentTargetPosition.y, CurrentTargetPosition.z)); //Get the distance between the current look position and the look goal (exluding the z axis)
                        EmeraldComponent.BehaviorsComponent.IsAiming = (CurrentLookDistance > 0.25f);
        ...
        {
             m_AimSourceParent.transform.rotation = Quaternion.Euler(0.0f, 0.0f, transform.rotation.z * -1.0f);
        }
        */
